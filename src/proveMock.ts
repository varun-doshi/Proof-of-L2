...
describe(`test prove ${pathfromfixtures}`, () => {
  ...
  it("test mock mode", async () => {
    ...
    const jsonRpcUrl = loadConfigByNetwork(
      yaml,
    ...
    const res = await cleapi.proveMock({ wasmUint8Array }, input);
  ...
  it.only(
    ...
    async () => {
      ...
      const result = await cleapi.waitProve(zkwasmUrl, taskId as string, {
